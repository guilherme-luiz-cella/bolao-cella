...
assert.ok(html.includes('id="finalScore"'));
assert.ok(html.includes('id="closestAnnouncement"'));
assert.ok(css.includes('.page'));
assert.ok(css.includes('.countdown-banner'));
assert.ok(css.includes('.result-card'));
...
assert.ok(js.includes('const VOTING_IS_CLOSED = true;'));
assert.ok(js.includes('class="ranking-name"'));
assert.ok(js.includes('renderFinalResult();'));
assert.ok(js.includes("from './results.js';"));
